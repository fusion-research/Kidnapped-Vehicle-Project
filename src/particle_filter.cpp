...
void ParticleFilter::resample() {
    ...
    // vector of resampled particles
    std::vector<Particle> resampled_particles;

    // keep previous weights
    ParticleFilter::keepWeights();
    double max_weight = *max_element(weights.begin(), weights.end());

    std::uniform_real_distribution<double> particle_dist(0.0, max_weight);
    std::uniform_real_distribution<int> index_dist(0.0, num_particles - 1);
    ...
    // set resampled particles to final particles
    particles = resampled_particles;
}

void ParticleFilter::keepWeights() {
    // keep previous weights
    weights.clear();
    for (int i = 0; i < num_particles; i++) {
